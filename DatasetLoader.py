...
class DatasetLoader(dataset):
    ...
    def load(self):
        ...
        features = sp.load_npz("{}/spX.npz".format(self.dataset_source_folder_path))

        # one_hot_labels = self.encode_onehot(idx_features_labels[:, -1])
    
        # build graph
        # idx = np.array(idx_features_labels[:, 0], dtype=np.int32)
        idx = np.array(range(features.shape[0]))
        ...
        #                                 dtype=np.int32)
        edges_unordered = np.load("{}/edge_index.npz".format(self.dataset_source_folder_path))['arr_0']
        edges_unordered = np.asarray([[i, j] for i,j in zip(edges_unordered[0],edges_unordered[1])])
        ...
        return {'X': features, 'A': adj, 'S': eigen_adj, 'index_id_map': index_id_map, 'edges': edges_unordered, 'raw_embeddings': raw_embeddings, 'wl_embedding': wl_embedding, 'hop_embeddings': hop_embeddings, 'int_embeddings': int_embeddings, 'y': [], 'idx': idx, 'idx_train': idx_train, 'idx_test': idx_test, 'idx_val': idx_val}
